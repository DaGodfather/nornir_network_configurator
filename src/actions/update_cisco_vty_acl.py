        ...
        if enable_secret:
            ...
            for attempt in range(2):  # Try twice
                try:
                    ...
                    # Always sync conn.secret from host.data["enable_secret"] — the connection
                    # may have been initialized with a stale secret (e.g. TACACS enable password
                    # when -use_local is active), causing enable() to send the wrong password
                    # and the device to close the SSH channel.
                        ...
                        if "telnet" in curr_dt_pw.lower():
                            ...
                            if _pw_ok:
                                ...
                                try:
                                    ...
                                    pass
                                # Don't sleep — retry immediately with updated password
                                continue
                            else:
                                logger.warning(
